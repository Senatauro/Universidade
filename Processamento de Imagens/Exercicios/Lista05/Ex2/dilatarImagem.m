...
%
function f = dilatarImagem(Matriz, ElementoEstruturante, Centro)
  ...
  size(ImagemAjustada)
  NMat = zeros(size(Matriz)(1) + (2 * size(ElementoEstruturante)(1)), size(Matriz)(2) + (2* size(ElementoEstruturante)(2)));
        ...
        for linhaE = 1:size(ElementoEstruturante)(1)
          for colunaE = 1:size(ElementoEstruturante)(2)
            if(ElementoEstruturante(linhaE, colunaE) == 0)
              NMat(linhaE + linha - centroX, coluna + colunaE - centroY) = 1;
            endif
          endfor
        endfor
      endif
  ...
  figure, imshow(NMat), title("Imagem Dilatada");
  f = NMat(1 + size(ElementoEstruturante)(1) : size(Matriz)(1) + size(ElementoEstruturante)(1),1 + size(ElementoEstruturante)(2) : size(Matriz)(2) + size(ElementoEstruturante)(2));

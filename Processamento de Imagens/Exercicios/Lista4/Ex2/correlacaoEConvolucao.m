function f = correlacaoEConvolucao(Imagem, Mascara, Divisor)
  ...
  %size(ImagemAumentada)
  %size(ImagemFinalConvolucao)
  %ImagemSomada = ImagemAumentada .+ ImagemFinalConvolucao;
  
  figure, imshow(uint8(ImagemFinalCorrelacao)), title("Correlacao");
  figure, imshow(uint8(ImagemFinalConvolucao)), title("Convolucao");
  f = uint8(ImagemFinalConvolucao);
  %figure, imshow(uint8(ImagemSomada)), title("Soma da imagem original com a convolucao");;
endfunction
...
void main()
{
    ...
    MostrarArvorePreOrdem(&arvore);
    printf("\n\nMostrando a arvore em ordem: ");
    MostrarArvoreEmOrdem(&arvore);
    printf("\n\nMostrando a arvore em pos-ordem");
    MostrarArvorePosOrdem(&arvore);
}
...
int BuscarElemento(Arvore_Int **arvore, int elemento)
{
    Arvore_Int *arvre = *arvore;
    if(arvre == NULL)
        return NULL;
